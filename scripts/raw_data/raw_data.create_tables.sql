/*
===============================================================================
DDL Script: Create raw data tables
===============================================================================
Script Purpose: 
    This script creates the tables for the raw_data schema, dropping existing ones if they already exist. 
===============================================================================
*/

-- Create ball_type_ref table
DROP TABLE IF EXISTS raw_data.ball_type_ref;
CREATE TABLE raw_data.ball_type_ref (
	ball_type 					VARCHAR(10) PRIMARY KEY, 	                                -- Ball type code
	ball_type_desc 				VARCHAR(35) 				                                -- Ball type full name
);

-- Create shot_type_ref table
DROP TABLE IF EXISTS raw_data.shot_type_ref;
CREATE TABLE raw_data.shot_type_ref (
...
-- Create player table
DROP TABLE IF EXISTS raw_data.player;
CREATE TABLE raw_data.player (
	  player_id 				VARCHAR(5) PRIMARY KEY, 	                                -- Unique player identifier
	  gender 					VARCHAR(1), 				                                -- F = Female, M = Male
	  dom_hand 					VARCHAR(1), 				                                -- Handedness, R = Right, L = Left
	  doubles_dupr 				FLOAT, 					                                    -- Doubles DUPR rating
	  doublesreliabilityscore 	FLOAT 					                                    -- Doubles DUPR reliability score
);

-- Create team table
DROP TABLE IF EXISTS raw_data.team;
CREATE TABLE raw_data.team (
	  team_id 					VARCHAR(5),				                                    -- Unique team identifier
	  player_id 				VARCHAR(5) REFERENCES raw_data.player (player_id),          -- Unique player identifier
	  player_seq_nbr 			INTEGER                                                     -- Differentiates two players on the same team
);

-- Create game table
DROP TABLE IF EXISTS raw_data.game;
CREATE TABLE raw_data.game (
...
-- Create rally table
DROP TABLE IF EXISTS raw_data.rally;
CREATE TABLE raw_data.rally (
...
-- Create shot table
DROP TABLE IF EXISTS raw_data.shot;
CREATE TABLE raw_data.shot (
	  shot_id 					VARCHAR(8) PRIMARY KEY,                                     -- Unique shot identifier
	  rally_id 					VARCHAR(6) REFERENCES raw_data.rally (rally_id),            -- Unique rally identifier
	  ...
	  player_id 				VARCHAR(5) REFERENCES raw_data.player (player_id),          -- The player who hit the shot, see the table description for more information
...
/*
DROP TABLE raw_data.ball_type_ref CASCADE;
DROP TABLE raw_data.game CASCADE;
DROP TABLE raw_data.player CASCADE;
DROP TABLE raw_data.rally CASCADE;
DROP TABLE raw_data.shot CASCADE;
DROP TABLE raw_data.shot_type_ref CASCADE;
DROP TABLE raw_data.team CASCADE;
*/
